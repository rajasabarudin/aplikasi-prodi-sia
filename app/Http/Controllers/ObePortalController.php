...
class ObePortalController extends Controller
{
    ...
    public function pdfRecap($kriteria, $ppepp)
    {
        ...
        if (str_starts_with($key, "C2_P2_Prestasi_")) {
            $bidang = str_replace("C2_P2_Prestasi_", "", $key);
            $bidang = str_replace("_", " ", $bidang);
            $title = "Laporan Prestasi Mahasiswa Bidang " . $bidang . " (Pelaksanaan C.2)";
            $rawData = \App\Models\PrestasiMahasiswa::with('mahasiswa')
                ->where('bidang_prestasi', $bidang)
                ->get();
            $data = $rawData->map(function($p) {
                return (object)[
                    'NIM' => $p->nim,
                    'Nama_Mahasiswa' => $p->mahasiswa->nama ?? '-',
                    'Nama_Prestasi' => $p->nama_prestasi,
                    'Tingkat' => $p->level_prestasi,
                    'Tahun' => $p->tahun
                ];
            });
