...
// Find the first occurrence of pdfRecap
$pos = strpos($content, '    public function pdfRecap($kriteria, $ppepp)');

if ($pos !== false) {
    // Truncate everything from that point onwards
    $top = substr($content, 0, $pos);
    
    $new_func = '    public function pdfRecap($kriteria, $ppepp)
    {
...
';
    ...
    file_put_contents('app/Http/Controllers/ObePortalController.php', $top . $new_func);
    echo "Fixed properly by truncating from first pdfRecap.";
} else {
    echo "Could not find pdfRecap.";
}
