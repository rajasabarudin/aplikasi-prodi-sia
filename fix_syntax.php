...
// Find where deletePpeppDocument ends
$target = "public function deletePpeppDocument(\$id)";
$pos = strpos($content, $target);

// Find the closing brace of deletePpeppDocument
$pos_return = strpos($content, "return redirect()->back()->with('success', 'Dokumen akreditasi berhasil dihapus!');", $pos);
$pos_brace = strpos($content, "}", $pos_return) + 1;

// Truncate the file at the end of deletePpeppDocument
$top = substr($content, 0, $pos_brace);
...
file_put_contents('app/Http/Controllers/ObePortalController.php', $final_content);
echo "Successfully nuked all garbage and appended pdfRecap.";
