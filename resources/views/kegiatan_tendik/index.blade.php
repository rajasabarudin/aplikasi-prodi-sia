...
@section('title', 'Data Kegiatan Tendik')
...
<div class="d-none d-print-block mb-4 text-center">
    <h4 class="fw-bold">Kegiatan Tendik Program Studi Sistem Informasi Akuntansi (D3)</h4>
    ...
    <div class="row">
        ...
        <div class="col-lg-9 col-md-8">
            <div class="d-flex justify-content-between align-items-center mb-3 d-print-none">
                <div>
                    <h1 class="mb-0 fw-bold text-dark" style="letter-spacing: -0.5px;">Data Kegiatan Tendik</h1>
                    ...
                    <form method="GET" class="row g-2 align-items-center">
                        <div class="col-md-5">
                            <input type="text" name="search" class="form-control glass-input"
                                placeholder="Cari Tendik, kegiatan, penyelenggara, level..."
                        ...
                        <div class="col-md-5 text-md-end">
                            ...
                            <a href="{{ route('kegiatan-tendik.index', array_merge(request()->query(), ['print' => 'all'])) }}"
                            ...
                            @if(request('search') || request('per_page'))
                                <a href="{{ route('kegiatan-tendik.index') }}" class="btn btn-secondary px-3" style="border-radius:10px;">Reset</a>
                            @endif
                        </div>
                    </form>
                </div>
                    ...
                    <tbody>
                        @forelse ($kegiatans as $item)
                            <tr>
                                ...
                                <td>
                                    <strong class="text-dark d-block" style="font-size:0.9rem;">{{ $item->nama_tendik }}</strong>
                                    <span class="badge bg-dark-subtle text-dark small" style="font-size:10px;">{{ $item->nip_nik }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    @if($item->jenis == 'Internal')
                                        @php
                                            $tendikModel = $item->Tendik;
                                            $identifiers = $tendikModel ? [$tendikModel->nip_nik] : [$item->nip_nik];
                                ...
                                <td class="text-center d-print-none">
                                    <div class="btn-group" role="group">
                                        <a href="{{ route('kegiatan-tendik.show', $item) }}"
                                           class="btn btn-sm btn-info text-white" style="border-radius:6px 0 0 6px;">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="{{ route('kegiatan-tendik.edit', $item) }}"
                                           class="btn btn-sm btn-warning">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <form action="{{ route('kegiatan-tendik.destroy', $item) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center text-muted py-4">
                                    ...
                                    Belum ada data Kegiatan Tendik.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<div class="modal fade" id="tambahkegiatanModal" tabindex="-1" aria-labelledby="tambahkegiatanModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content glass-panel border-0">
            <form action="{{ route('kegiatan-tendik.store') }}" method="POST">
                @csrf
                <div class="modal-header gradient-header">
                    <h5 class="modal-title fw-bold" id="tambahkegiatanModalLabel">
                        <i class="bi bi-plus-circle-fill me-2"></i>Tambah Kegiatan Tendik
                    </h5>
                ...
                <div class="modal-body p-4">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="modal_nip_nik" class="form-label fw-semibold">Kode Tendik <span class="text-danger">*</span></label>
                            <select name="nip_nik" id="modal_nip_nik" class="form-select glass-input"
                                    data-route="{{ route('kegiatan-tendik.get-tendik', '') }}/" required>
                                <option value="">-- Pilih Kode Tendik --</option>
                                @foreach ($Tendiks as $Tendik)
                                    <option value="{{ $Tendik->nip_nik }}">
                        ...
                        <div class="col-md-6 mb-3">
                            <label for="modal_nama_tendik" class="form-label fw-semibold">Nama Tendik <span class="text-danger">*</span></label>
                            <input type="text" name="nama_tendik" id="modal_nama_tendik" class="form-control glass-input" readonly required>
                        </div>
                        <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-12 mb-3" id="wrapper_kegiatan_internal" style="display: none;">
                            ...
                            <small class="text-muted">Sertifikat akan otomatis diambil dari sistem jika Tendik ini terdaftar sebagai peserta.</small>
                        </div>
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    // Autocomplete Kode Tendik
    document.getElementById('modal_nip_nik').addEventListener('change', function () {
        var kode = this.value;
        if (kode) {
            var route = this.getAttribute('data-route');
            fetch(route + kode)
                .then(res => res.json())
                .then(data => { document.getElementById('modal_nama_tendik').value = data.nama_tendik; })
                .catch(() => { document.getElementById('modal_nama_tendik').value = ''; });
        } else {
            document.getElementById('modal_nama_tendik').value = '';
        }
