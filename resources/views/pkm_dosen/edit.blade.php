...
@section('title', 'Edit PKM Dosen')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1>Edit PKM Dosen</h1>
    ...
    <form action="{{ route('pkm-dosen.update', $pkmDosen->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card p-4 mb-4">
            ...
            <div class="mb-3">
                ...
                <div id="dosen-rows-container">
                    @php
                        $kodeDosens = explode(', ', $pkmDosen->kode_dosen);
                        $namaDosens = explode(', ', $pkmDosen->nama_dosen);
                    @endphp
                    @foreach ($kodeDosens as $i => $kd)
                    <div class="row g-2 mb-2 dosen-row">
                        <div class="col-md-5">
                            <select name="kode_dosen[]" class="form-select select-kode-dosen" data-route="{{ route('pkm-dosen.get-dosen', '') }}/" required>
                                ...
                                @foreach ($dosens as $dosen)
                                    <option value="{{ $dosen->kode_dosen }}" {{ $kd == $dosen->kode_dosen ? 'selected' : '' }}>
                        ...
                        <div class="col-md-5">
                            <input type="text" name="nama_dosen[]" class="form-control input-nama-dosen" readonly required placeholder="Nama Dosen" value="{{ $namaDosens[$i] ?? '' }}">
                        </div>
                        <div class="col-md-2">
                            @if (!$loop->first)
        ...
        <div class="card p-4 mb-4">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="tema_pkm" id="tema_pkm" class="form-control" value="{{ old('tema_pkm', $pkmDosen->tema_pkm) }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="mitra" class="form-label">Mitra <span class="text-danger">*</span></label>
                    <input type="text" name="mitra" id="mitra" class="form-control" value="{{ old('mitra', $pkmDosen->mitra) }}" required>
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select name="jenis_pkm" id="jenis_pkm" class="form-select" required>
                        ...
                        <option value="Mitra Non Produktif" {{ old('jenis_pkm', $pkmDosen->jenis_pkm) == 'Mitra Non Produktif' ? 'selected' : '' }}>Mitra Non Produktif</option>
                        <option value="Mitra Produktif" {{ old('jenis_pkm', $pkmDosen->jenis_pkm) == 'Mitra Produktif' ? 'selected' : '' }}>Mitra Produktif</option>
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="sumber_iptek" class="form-label">Sumber IPTEK <span class="text-danger">*</span></label>
                    <input type="text" name="sumber_iptek" id="sumber_iptek" class="form-control" value="{{ old('sumber_iptek', $pkmDosen->sumber_iptek) }}" required>
                </div>
            </div>
        </div>
        ...
        <div class="card p-4 mb-4">
            ...
            <div class="mb-3">
                ...
                <div id="mahasiswa-rows-container">
                    @php
                        $nimMhsList = $pkmDosen->nim_mhs ? explode(', ', $pkmDosen->nim_mhs) : [];
                        $namaMhsList = $pkmDosen->nama_mahasiswa ? explode(', ', $pkmDosen->nama_mahasiswa) : [];
                    @endphp
                    @forelse ($nimMhsList as $j => $nim)
                    <div class="row g-2 mb-2 mahasiswa-row">
                        <div class="col-md-5">
                            <input type="text" name="nim_mhs[]" class="form-control input-nim-mhs" data-route="{{ route('pkm-dosen.get-mahasiswa', '') }}/" value="{{ $nim }}" placeholder="Masukkan NIM...">
                        </div>
                        <div class="col-md-5">
                            <input type="text" name="nama_mahasiswa[]" class="form-control input-nama-mahasiswa" readonly placeholder="Nama Mahasiswa" value="{{ $namaMhsList[$j] ?? '' }}">
                        </div>
                        <div class="col-md-2">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="ts_id" id="ts_id" class="form-select" required>
                        ...
                        @foreach ($tsList as $ts)
                            <option value="{{ $ts->id }}" {{ old('ts_id', $pkmDosen->ts_id) == $ts->id ? 'selected' : '' }}>{{ $ts->tahun_sekarang }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6 mb-3">
                    <label for="link_dokumen" class="form-label">Link Dokumen</label>
                    <input type="url" name="link_dokumen" id="link_dokumen" class="form-control" value="{{ old('link_dokumen', $pkmDosen->link_dokumen) }}" placeholder="https://example.com/dokumen">
                </div>
                <div class="col-md-6 mb-3">
                    <label for="link_publikasi" class="form-label">Link Publikasi</label>
                    <input type="url" name="link_publikasi" id="link_publikasi" class="form-control" value="{{ old('link_publikasi', $pkmDosen->link_publikasi) }}" placeholder="https://example.com/publikasi">
                </div>
            </div>
        </div>

        <div class="mb-4 text-end">
            <button type="submit" class="btn btn-primary btn-lg px-5"><i class="bi bi-save"></i> Update Data</button>
        </div>
    </form>
</div>
